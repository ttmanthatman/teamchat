...
<head>
  ...
  <title>团队聊天室</title>
  <link rel="stylesheet" href="style.css?v=20260326b">
</head>
<body>
  ...
  <div id="chatPage" class="page hidden">
    ...
    <div class="rich-toolbar" id="richToolbar" onmousedown="event.preventDefault()" ontouchstart="">
      ...
      <span class="tb-sep"></span>
      <button type="button" class="tb-btn tb-chain-btn" onclick="showChainDialog()" title="发起接龙">🚂</button>
    </div>
    <div class="input-area">
      <button onmousedown="event.preventDefault()" onclick="document.getElementById('fileInput').click()" class="attach-btn">📎</button>
  ...
  <script src="app.js?v=20260326b"></script>
</body>
</html>
HTMLEOF
...
/* 接龙 (Chain) styles */
.tb-chain-btn{font-size:16px!important;position:relative}
...
(function(){
  ...
  document.addEventListener('focusout',function(e){
    // 如果焦点移动到工具栏或输入区域内的按钮，不要触发滚动归位
    var related=e.relatedTarget;
    if(related){
      var toolbar=document.getElementById('richToolbar');
      var inputArea=document.querySelector('.input-area');
      if((toolbar&&toolbar.contains(related))||(inputArea&&inputArea.contains(related)))return;
    }
  ...
  if(window.visualViewport){
    ...
    window.visualViewport.addEventListener('resize',function(){
      ...
      if(newHeight>lastHeight){
        ...
        var toolbar=document.getElementById('richToolbar');
        var inputArea=document.querySelector('.input-area');
        if(ae&&((toolbar&&toolbar.contains(ae))||(inputArea&&inputArea.contains(ae)))){
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  (function(){
    ...
    function preventFocusSteal(el){
      ...
      // 也阻止容器上的 mousedown 和 touchstart
    ...
    preventFocusSteal(toolbar);
    if(inputArea){
      // 只对按钮做，不影响 contenteditable 输入框本身
      inputArea.querySelectorAll('button').forEach(function(btn){
        btn.addEventListener('touchstart',function(e){e.preventDefault()},{passive:false});
...
function execFmt(cmd,val){var inp=document.getElementById('messageInput');document.execCommand(cmd,false,val||null);inp.focus()}
